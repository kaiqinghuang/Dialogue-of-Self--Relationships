...
namespace com.zibra.smoke_and_fire.Solver
{
    ...
    [ExecuteInEditMode]
    public class ZibraSmokeAndFire : MonoBehaviour, StatReporter
    {
...
#region Render functions
        ...
        private void SetMaterialParams(Material material, Vector2 resolution)
        {
            ...
            material.SetVector("ShadowGridSize", (Vector3)ShadowGridSize);
            material.SetVector("LightGridSize", (Vector3)LightGridSize);
            ...
            material.SetTexture("ParticlesTex", ParticlesRT);
            ...
            material.SetFloat("deltaT", dt);
            material.SetFloat("StepScale", stepScale);
            material.SetVector("ContainerMinPoint", new Vector3(SimulationContainerPosition.x - ContainerSize.x * 0.5f, 
                SimulationContainerPosition.y - ContainerSize.y * 0.5f, SimulationContainerPosition.z - ContainerSize.z * 0.5f));
            material.SetVector("ContainerMaxPoint", new Vector3(SimulationContainerPosition.x + ContainerSize.x * 0.5f,
                SimulationContainerPosition.y + ContainerSize.y * 0.5f, SimulationContainerPosition.z + ContainerSize.z * 0.5f));
        }
        ...
        private bool SetMaterialParams(Camera cam)
        {
            ...
            isDirty = camRes.SmokeShadowProjectionMaterial.SetMaterial(CurrenSharedShadowProjectionMaterial) || isDirty;
            ...
            {
                if (!EnableDownscale && !cam.stereoEnabled)
                {
                    CurrentMaterial.EnableKeyword("FLIP_NATIVE_TEXTURES");
                }
                else
            ...
            if (VisualizeSceneSDF)
            {
                ...
                CurrentSDFRenderMaterial.SetTexture("SDFRenderSmoke", VisualizeSDFTarget);

                if (RenderPipelineDetector.GetRenderPipelineType() == RenderPipelineDetector.RenderPipeline.HDRP)
                {
#if UNITY_PIPELINE_HDRP
                    CurrentSDFRenderMaterial.SetVector("LightColor", MainLight.color *
        ...
        private void UpdateCamera(Camera cam)
        {
            ...
            if (IsCameraInside)
            {
                CurrentMaterial.EnableKeyword("FULLSCREEN_QUAD");
            }
            else
            {
                CurrentMaterial.DisableKeyword("FULLSCREEN_QUAD");
            }
